...
export function formalize(ranges, options, schema) {
	...
	return ranges
		...
		.map((range) => {
			...
			if (boundaries.length != 2) throw new SchemaError(schema, 'Malformed range \'' + range + '\'.');

			// Test for malformed boundaries
			for (let bIdx = 0 ; bIdx < 2 ; bIdx++) {
				if (!/^\(?[-0-9.]*\)?$/.test(boundaries[bIdx])) throw new SchemaError(schema, 'Malformed boundary \'' + boundaries[bIdx] + '\'.');
			}
			...
			if ((options || {}).allowNegative === false && (lower < 0 || upper < 0)) {
				throw new SchemaError(schema, 'Boundary cannot be a negative value.');
			}

			if ((options || {}).allowNonInteger === false && (!Number.isInteger(lower) || !Number.isInteger(upper))) {
				throw new SchemaError(schema, 'Boundary cannot be a non-integer value.');
			}
			...
			if (lower > upper) {
				throw new SchemaError(schema, 'Malformed range \'' + range +'\'');
			}
