...
var validateCustom = function(data, schema, fn, keyPath, options) {
	...
	var finalize = function(data, fn) {
		if (fn) return utils.asap(fn, null, data);
	};
...
var validateAny = function(data, schema, fn, keyPath, options) {
	...
	if (typeof schema.equal !== 'undefined' || typeof (options || {}).equal !== 'undefined') {
		if (!equals(schema.equal || (options || {}).equal, data)) {
			return fn(
				new ValidationError(
					keyPath,
					schema._nonFormalizedSchema,
	...
	if ('Object' == utils.name(schema.type)) return validateObject(data, schema, fn, keyPath, options);
	if ('Array' == utils.name(schema.type)) return validateArray(data, schema, fn, keyPath, options);
	if ('String' == utils.name(schema.type)) return validateString(data, schema, fn, keyPath, options);
	if ('Number' == utils.name(schema.type)) return validateNumber(data, schema, fn, keyPath, options);
	if ('Boolean' == utils.name(schema.type)) return validateBoolean(data, schema, fn, keyPath, options);
	if ('Date' == utils.name(schema.type)) return validateDate(data, schema, fn, keyPath, options);

	// This error should have been eliminated by the formalizer.
	throw new Error('Cannot validate schema of type ' + utils.name(schema.type) + '.');

};

module.exports = function(data, schema, fn, keyPath, defaults) {

	var options = data;

	if (schema || fn || keyPath || defaults) {

		if (defaults === undefined && typeof keyPath == 'object' && utils.name(keyPath.constructor) == 'Object') {
			defaults = keyPath;
			keyPath = undefined;
		}
	...
	if (typeof options.schema === 'undefined') throw new Error('Missing parameter schema.');
	...
	if (typeof (options.defaults || {}).allowUnknownKeys === 'boolean') {
		...
		options.defaults.unknownKeys = (options.defaults.allowUnknownKeys ? 'allow' : 'deny');
		options.defaults.wasAllowUnknownKeys = true;
		delete options.defaults.allowUnknownKeys;
	}
	...
	if (!options.fn && global.Promise) {
		return new Promise(function(resolv, reject) {
			validateAny(options.data, options.schema, function(err, data) {
				if (err) return reject(err);
				resolv(data);
			}, options.keyPath, options.defaults);
		});
	}

	return validateAny(options.data, options.schema, options.fn, options.keyPath, options.defaults);
