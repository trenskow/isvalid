...
const validateCustom = async (data, schema, options, keyPath) => {
	...
	if (schema.custom === undefined) return data;

	for (let idx = 0 ; idx < schema.custom.length ; idx++) {
		try {
			let result = await Promise.resolve(schema.custom[idx](data, schema, options));
			data = result || data;
		} catch (error) {
			throw ValidationError.fromError(keyPath, schema._nonFormalizedSchema, 'custom', error);
		}
	}
