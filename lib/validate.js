...
import { formalize as formalizeRange, testIndex } from './ranges.js';
...
const validateNumber = async (data, schema, options, keyPath) => {
	...
	if (schema.range || options.defaults._formalizedRange) {
		if (!testIndex(schema.range || options.defaults._formalizedRange, data)) {
			throw new ValidationError(
				keyPath,
				schema._nonFormalizedSchema,
				'range',
...
const validateCustom = async (phase, data, schema, options, keyPath, validatedData) => {
	...
	let validators = schema[phase] || [];
	let defaultValidators = options.defaults[phase] || [];

	if (!Array.isArray(validators)) {
		validators = [validators].filter((item) => typeof item === 'function');
	}

	if (!Array.isArray(defaultValidators)) {
		defaultValidators = [defaultValidators].filter((item) => typeof item === 'function');
	}

	validators = defaultValidators.concat(validators);

	if (!validators.length) return data;

	for (let idx = 0 ; idx < validators.length ; idx++) {
		try {
			let result = await Promise.resolve(validators[idx](data, schema, { options, keyPath, data: validatedData }));
...
const validateAny = async (data, schema, options, keyPath, validatedData) => {
	...
	if (schema._nonFormalizedSchema === undefined) {
		return await validateAny(data, (options.formalize || formalize)(schema, options), options, keyPath, validatedData);
	}
...
export default async (data, schema, options = {}, keyPath = '') => {
	...
	if (options.defaults.range) {
		Object.defineProperty(options.defaults, '_formalizedRange', {
			value: formalizeRange(options.defaults.range, options, schema),
			writable: false,
			enumerable: false,
			configurable: false
		});
