...
const validateObject = async (data, schema, options, keyPath) => {
	...
	return await validateCustom(data, schema, options, keyPath);
...
const validateCustom = async (data, schema, options, keyPath) => {

	// All other validators always end with calling the post validator.
	// - therefore just finalize if there is none.
	if (schema.post === undefined) return data;

	for (let idx = 0 ; idx < schema.post.length ; idx++) {
		try {
			let result = await Promise.resolve(schema.post[idx](data, schema, options));
			data = result || data;
		} catch (error) {
			throw ValidationError.fromError(keyPath, schema._nonFormalizedSchema, 'post', error);
		}
	}
...
const validateAny = async (data, schema, options, keyPath) => {
	...
	if (schema.type === undefined) return await validateCustom(data, schema, options, keyPath);
	if ('Object' == utils.name(schema.type)) return await validateObject(data, schema, options, keyPath);
	if ('Array' == utils.name(schema.type)) return await validateArray(data, schema, options, keyPath);
	if ('String' == utils.name(schema.type)) return await validateString(data, schema, options, keyPath);
	if ('Number' == utils.name(schema.type)) return await validateNumber(data, schema, options, keyPath);
	if ('Boolean' == utils.name(schema.type)) return await validateBoolean(data, schema, options, keyPath);
	if ('Date' == utils.name(schema.type)) return await validateDate(data, schema, options, keyPath);
...
module.exports = exports = async (data, schema, options = {}, keyPath = []) => {
	...
	return await validateAny(data, schema, options, keyPath);
