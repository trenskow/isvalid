...
const formalizeObject = async (formalizedSchema, nonFormalizedSchema) => {
	...
	return await finalize(formalizedSchema);
...
const formalizeAny = async (schema) => {
	...
	if (!schema.type && !schema.post && !schema.equal) {
		if ('Object' == utils.name(schema.constructor)) {
			return await formalizeObject({ type: Object, schema: schema });
		}
		if ('Array' == utils.name(schema.constructor)) {
			if (schema.length === 0) throw new SchemaError(schema, 'Array must have exactly one schema.');
			return await formalizeArray({ type: Array, schema: schema[0] });
		}
		if (typeof schema === 'function' && supportedTypes.indexOf(utils.name(schema)) > -1) {
			return await formalizeAny({ type: schema });
		}
		throw new SchemaError(schema, 'Schemas must have at least on validator of `type`, `post` and/or `equal`.');
	}
	...
	if (formalizedSchema.post !== undefined) {
