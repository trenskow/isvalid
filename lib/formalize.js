...
const formalizeAny = async (schema) => {
	...
	if (!schema.type && !schema.post && !schema.pre && !schema.equal) {
		...
		if (typeof schema === 'function' && utils.typeName(schema) !== undefined) {
	...
	if (formalizedSchema.type !== undefined) {
		if ('Object' == utils.typeName(formalizedSchema.type)) merge(validators, {
			'schema': 'Any',
			'unknownKeys': [ 'String' ]
		});
		if ('Array' == utils.typeName(formalizedSchema.type)) merge(validators, {
			'schema': 'Any',
			'len': [ 'String', 'Number' ],
			'unique': [ 'Boolean' ],
			'autowrap': [ 'Boolean' ]
		});
		if ('String' == utils.typeName(formalizedSchema.type)) merge(validators, {
			'match': [ 'RegExp' ],
			'trim': [ 'Boolean' ],
			'enum': [ 'Array' ]
		});
		if ('Number' == utils.typeName(formalizedSchema.type)) merge(validators, {
			'range': [ 'String', 'Number' ]
		});
	}
	...
	for (let key in formalizedSchema) {
		...
		if (utils.instanceTypeName(test) === 'Array' &&
			test.length === 2 &&
			(validator === 'Any' || validator.indexOf(utils.instanceTypeName(test[0])) > -1)) {
	...
	if (formalizedSchema.type) {
		if ('Object' == utils.typeName(formalizedSchema.type)) {
			return await formalizeObject(formalizedSchema, schema);
		}
		if ('Array' == utils.typeName(formalizedSchema.type)) {
