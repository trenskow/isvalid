...
const formalizeObject = (formalizedSchema, nonFormalizedSchema) => {
	...
	Object.keys(formalizedSchema.schema)
		.forEach((key) => {
			...
			const formalizedKey = formalizeAny(formalizedSchema.schema[key]);
...
const formalizeArray = (formalizedSchema, nonFormalizedSchema) => {
	...
	formalizedSchema.schema = formalizeAny(formalizedSchema.schema);
...
const formalizeAny = (schema) => {
	...
	if (formalizedSchema.type) {
		if (utils.isSameType('Object', utils.typeName(formalizedSchema.type))) {
			return formalizeObject(formalizedSchema, schema);
		}
		if (utils.isSameType('Array', utils.typeName(formalizedSchema.type))) {
			return formalizeArray(formalizedSchema, schema);
		}
	}
	...
	Object.keys(formalizedSchema.errors || {})
		.forEach((key) => {
			if (typeof formalizedSchema.errors[key] !== 'string') {
				throw new SchemaError(
					schema,
					'Custom error message for validator \'' + key + '\' must be of type `String`.'
				);
			}
		});
