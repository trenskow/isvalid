...
const formalizeAny = (schema, options = {}) => {
	...
	for (let key in formalizedSchema) {
		...
		if (typeof (options.transform || {}).pre === 'function') {
			formalizedSchema[key] = options.transform.pre(key, formalizedSchema[key], { schema: formalizedSchema }) || formalizedSchema[key];
		}
		...
		if (typeof (options.transform || {}).post === 'function') {
			formalizedSchema[key] = options.transform.post(key, formalizedSchema[key], { schema: formalizedSchema }) || formalizedSchema[key];
		}
